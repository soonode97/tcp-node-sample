...
public class NetworkManager : MonoBehaviour
{
    ...
    async void SendPingPacket(long timestamp) {
        // Ping 클래스에 따라 timestamp만 포함하여 전송
        Ping ping = new Ping { timestamp = timestamp };
    ...
    void ProcessReceivedData(byte[] data, int length) {
        ...
        while (incompleteData.Count >= 5) {
            ...
            if (packetLength < 5 || incompleteData.Count < packetLength) {
                return;
            }
    ...
    void HandlePingPacket(byte[] packetData) {
        // Ping 패킷을 디시리얼라이즈하여 timestamp 확인
        Ping receivedPing = Packets.Deserialize<Ping>(packetData);
        Debug.Log($"Received PING: timestamp = {receivedPing.timestamp}");

        // 서버로부터 올바른 응답을 받았을 때 핑 패킷 재전송
        TimeSpan timeSpan = (DateTime.UtcNow - new DateTime(1970, 1, 1, 0, 0, 0));
        SendPingPacket((long)timeSpan.TotalMilliseconds);
    }
    ...
    void HandleNormalPacket(byte[] packetData) {
        ...
        if (response.data != null && response.data.Length > 0) {
            string responseData = ProcessResponseData(response.data);
            if (response.handlerId == 0) {
                var locationData = JsonUtility.FromJson<LocationUpdateResponse>(responseData);
                Debug.Log(locationData.x);
                Debug.Log(locationData.y);
                GameManager.instance.GameStart(locationData.x, locationData.y);
            }
        }
    }

    string ProcessResponseData(byte[] data) {
        try {
